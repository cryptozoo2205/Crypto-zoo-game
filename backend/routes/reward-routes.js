...
router.post("/api/withdraw/request", async (req, res) => {
    ...
    const updatedPlayer = applyCreateWithdrawToPlayer(player, amount);
    if (!updatedPlayer) {
        return res.status(400).json({
...
router.post("/api/withdraw/update", async (req, res) => {
    ...
    const processingValidation = validateWithdrawProcessing(player, request);
    if (!processingValidation.ok) {
        return res.status(400).json({
            error: processingValidation.error
        });
    }
    ...
    if (nextStatus === "paid") {
        ...
        if (!updatedPlayer) {
            return res.status(400).json({
                error: "Failed to apply paid withdraw"
            });
        }

        const markedRequest = markWithdrawAsPaid(request, note, payoutTxHash);
        if (!markedRequest) {
            return res.status(400).json({
                error: "Failed to mark withdraw as paid"
            });
        }
    }
    ...
    if (nextStatus === "rejected") {
        ...
        if (!updatedPlayer) {
            return res.status(400).json({
                error: "Failed to reject withdraw"
            });
        }

        const markedRequest = markWithdrawAsRejected(request, note);
        if (!markedRequest) {
            return res.status(400).json({
                error: "Failed to mark withdraw as rejected"
            });
        }
    }
