...
function getLegacyTimeBoostCharges(player) {
    ensureExpeditionStats(player);
...
function setLegacyTimeBoostCharges(player, charges) {
    ensureExpeditionStats(player);
    player.expeditionStats.timeBoostCharges = Array.isArray(charges) ? charges : [];
}

function getTimeBoostCharges(player) {
    return getLegacyTimeBoostCharges(player);
}

function setTimeBoostCharges(player, charges) {
    setLegacyTimeBoostCharges(player, charges);
}

function getShopTimeBoostChargeCount(player, itemId) {
    if (!player || typeof player !== "object") return 0;
    player.shopItemCharges = player.shopItemCharges && typeof player.shopItemCharges === "object"
        ? player.shopItemCharges
        : {};
    return Math.max(0, Number(player.shopItemCharges[itemId]) || 0);
}

function getAvailableTimeBoostOptions(player) {
    const options = [];
    const shopMappings = [
        { itemId: "expeditionTime1", seconds: 600 },
        { itemId: "expeditionTime2", seconds: 1800 },
        { itemId: "expeditionTime3", seconds: 3600 },
        { itemId: "expeditionTime4", seconds: 14400 }
    ];

    shopMappings.forEach(({ itemId, seconds }) => {
        const count = getShopTimeBoostChargeCount(player, itemId);
        if (count > 0) {
            options.push({
                source: "shop",
                itemId,
    ...
    getLegacyTimeBoostCharges(player).forEach((value) => {
        const seconds = Math.max(0, Number(value) || 0);
        if (seconds <= 0) return;
        legacyCounts.set(seconds, (legacyCounts.get(seconds) || 0) + 1);
    });
...
function consumeSpecificTimeBoostCharge(player, requestedSeconds, remainingSeconds) {
    const safeRequestedSeconds = Math.max(0, Math.floor(Number(requestedSeconds) || 0));
    const safeRemaining = Math.max(0, Number(remainingSeconds) || 0);
    const maxAllowedReduction = Math.max(0, safeRemaining - 60);

    if (safeRequestedSeconds <= 0 || maxAllowedReduction <= 0) {
        return 0;
    }
    ...
    if (selectedOption.source === "shop" && selectedOption.itemId) {
        player.shopItemCharges = player.shopItemCharges && typeof player.shopItemCharges === "object"
            ? player.shopItemCharges
            : {};

        const currentCount = getShopTimeBoostChargeCount(player, selectedOption.itemId);
        if (currentCount <= 0) {
            return 0;
        }

        player.shopItemCharges[selectedOption.itemId] = currentCount - 1;
        return appliedReduction;
    }

    const legacyCharges = getLegacyTimeBoostCharges(player);
    const index = legacyCharges.findIndex((value) => {
        return Math.max(0, Number(value) || 0) == safeRequestedSeconds;
    });
    ...
    legacyCharges.splice(index, 1);
    setLegacyTimeBoostCharges(player, legacyCharges);
...
function consumeBestTimeBoostCharge(player, remainingSeconds) {
    ...
    options.forEach((option) => {
        const safeSeconds = Math.max(0, Number(option?.seconds) || 0);
        const applied = Math.min(safeSeconds, maxAllowedReduction);
    ...
    if (bestSeconds <= 0 || bestApplied <= 0) {
        return 0;
    }

    return consumeSpecificTimeBoostCharge(player, bestSeconds, safeRemaining);
}
...
router.post("/time-boost", (req, res) => {
    try {
        ...
        const reductionSeconds = requestedSeconds > 0
            ...
            : consumeBestTimeBoostCharge(player, remainingSeconds);
